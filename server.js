...
// 👨‍👩‍👧‍👦 YENİ: Öğrenci şemasına 'veliKodu' ve 'aktifDuello' eklendi
...
// 🎓 Kayıt Olurken Veli Kodu Üretiliyor
app.post('/api/kayit', async (req, res) => {
    try {
        ...
        let vKodu = 'V-' + Math.floor(1000 + Math.random() * 9000); // Örn: V-4821
        let yeniOgrenci = new Ogrenci({ ogrenciAd, sifre, kocKodu, veliKodu: vKodu, xp: 0, avatar: '👤', gorevler: [], istatistik: {}, netler: [], alinanOduller: [], aktifDuello: null }); await yeniOgrenci.save(); res.json({ basari: true, mesaj: "Kayıt başarılı!" });
...
app.post('/api/veli/giris', async (req, res) => {
    try { let ogrenci = await Ogrenci.findOne({ veliKodu: req.body.veliKodu }); if(ogrenci) res.json({ basari: true, ogrenciAd: ogrenci.ogrenciAd, kocKodu: ogrenci.kocKodu }); else res.json({ basari: false, mesaj: "Geçersiz Veli Kodu!" }); } catch(e) { res.json({ basari: false }); }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('duello_kabul_edildi', async (veri) => {
        try {
            let o1 = await Ogrenci.findOne({ ogrenciAd: veri.gonderen, kocKodu: veri.kocKodu }); let o2 = await Ogrenci.findOne({ ogrenciAd: veri.hedef, kocKodu: veri.kocKodu });
            if(o1 && o2) {
                let dData = { rakip: veri.hedef, miktar: veri.miktar }; let dData2 = { rakip: veri.gonderen, miktar: veri.miktar };
                o1.aktifDuello = dData; o2.aktifDuello = dData2; await o1.save(); await o2.save();
                io.to(veri.kocKodu).emit('gorev_guncellendi', await Ogrenci.find({kocKodu: veri.kocKodu})); io.to(veri.kocKodu).emit('duello_basladi', { o1: veri.gonderen, o2: veri.hedef });
            }
        } catch(e){}
    });
    ...
    // ⚔️ Düello Sonuçlandırma (İlk Görevi Bitiren Kazanır)
                ...
                if (gIndex !== -1 && ogrenci.gorevler[gIndex].tamamlandi === false) {
                    ...
                    if(ogrenci.aktifDuello && ogrenci.aktifDuello.rakip) {
                        ...
                        if(rakipOgrenci) {
                            ...
                            ogrenci.xp += kazanilanXP; rakipOgrenci.xp -= kazanilanXP;
                            if(rakipOgrenci.xp < 0) rakipOgrenci.xp = 0;
                            rakipOgrenci.aktifDuello = null; await rakipOgrenci.save();
