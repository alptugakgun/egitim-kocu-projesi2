...
// 👑 YENİ: Gelişmiş Admin Paneli (Veri Listeleme ve Hoca Banlama)
app.post('/api/admin', async (req, res) => {
    const { sifre } = req.body;
    if(sifre === 'sincap-boss-2026') {
        let koclar = await Ogretmen.find(); let ogrler = await Ogrenci.find();
        let detayliListe = koclar.map(k => { let ogrenciSayisi = ogrler.filter(o => o.kocKodu === k.kocKodu).length; return { isim: k.kocAd, kod: k.kocKodu, ogrenciSayisi: ogrenciSayisi }; });
        res.json({ basari: true, data: { koc: koclar.length, ogr: ogrler.length, liste: detayliListe } });
...
app.post('/api/admin/sil', async (req, res) => {
    ...
    if(sifre === 'sincap-boss-2026') {
        ...
        await Ogrenci.deleteMany({ kocKodu: kod }); // Hocanın tüm sınıfını da temizler
...
app.post('/api/koc/kayit', async (req, res) => {
    try {
        const { kocAd, sifre } = req.body; let varMi = await Ogretmen.findOne({ kocAd }); if (varMi) return res.json({ basari: false, mesaj: "Bu isimde bir öğretmen zaten var!" });
        let yeniKod = Math.random().toString(36).substr(2, 6).toUpperCase();
        let yeniKoc = new Ogretmen({ kocAd, sifre, kocKodu: yeniKod }); await yeniKoc.save();
...
app.post('/api/koc/giris', async (req, res) => {
    try { const { kocAd, sifre } = req.body; let koc = await Ogretmen.findOne({ kocAd, sifre }); if (koc) res.json({ basari: true, mesaj: "Giriş başarılı!", kocKodu: koc.kocKodu }); else res.json({ basari: false, mesaj: "İsim veya şifre hatalı!" }); } 
    catch (e) { res.json({ basari: false, mesaj: "Sunucu hatası!" }); }
});

app.post('/api/kayit', async (req, res) => {
    try {
        const { ogrenciAd, sifre, kocKodu } = req.body; let kocVarMi = await Ogretmen.findOne({ kocKodu }); if(!kocVarMi) return res.json({ basari: false, mesaj: "Böyle bir Davet Kodu bulunamadı!" });
        let varMi = await Ogrenci.findOne({ ogrenciAd, kocKodu }); if (varMi) return res.json({ basari: false, mesaj: "Bu isimde bir öğrenci sınıfınızda zaten var!" });
        let yeniOgrenci = new Ogrenci({ ogrenciAd, sifre, kocKodu, xp: 0, avatar: '👤', gorevler: [], istatistik: {}, netler: [], alinanOduller: [] }); await yeniOgrenci.save(); res.json({ basari: true, mesaj: "Kayıt başarılı! Giriş yapabilirsiniz." });
    } catch (e) { res.json({ basari: false, mesaj: "Sunucu hatası!" }); }
});

app.post('/api/giris', async (req, res) => {
    try { const { ogrenciAd, sifre } = req.body; let ogrenci = await Ogrenci.findOne({ ogrenciAd, sifre }); if (ogrenci) res.json({ basari: true, mesaj: "Giriş başarılı!", kocKodu: ogrenci.kocKodu }); else res.json({ basari: false, mesaj: "İsim veya şifre hatalı!" }); } 
    catch (e) { res.json({ basari: false, mesaj: "Sunucu hatası!" }); }
});

app.post('/api/sifreler', async (req, res) => { try { let ogrenciler = await Ogrenci.find({ kocKodu: req.body.kocKodu }, 'ogrenciAd sifre -_id'); res.json(ogrenciler); } catch (e) { res.json([]); } });

io.on('connection', (socket) => {
    socket.on('join_room', async (kocKodu) => {
        socket.join(kocKodu);
        try {
            const ogrenciler = await Ogrenci.find({ kocKodu }); socket.emit('eski_verileri_yukle', ogrenciler); socket.emit('gorev_guncellendi', ogrenciler);
            const eskiChat = await Chat.find({ kocKodu }).sort({_id: -1}).limit(50); socket.emit('eski_chat_yukle', eskiChat.reverse());
            const eskiKaynaklar = await Kaynak.find({ kocKodu }).sort({id: -1}); socket.emit('kaynaklari_yukle', eskiKaynaklar);
        } catch (hata) {}
    });
    ...
    socket.on('gorev_tamamlandi', async (veri) => { try { let ogrenci = await Ogrenci.findOne({ ogrenciAd: veri.ogrenciAd, kocKodu: veri.kocKodu }); if (ogrenci && ogrenci.gorevler) { let gIndex = ogrenci.gorevler.findIndex(g => Number(g.id) === Number(veri.gorevId)); if (gIndex !== -1 && ogrenci.gorevler[gIndex].tamamlandi === false) { ogrenci.gorevler[gIndex].tamamlandi = true; ogrenci.xp = Number(ogrenci.xp || 0) + 10; ogrenci.markModified('gorevler'); await ogrenci.save(); io.to(veri.kocKodu).emit('gorev_guncellendi', await Ogrenci.find({kocKodu: veri.kocKodu})); } } } catch(e){} });
