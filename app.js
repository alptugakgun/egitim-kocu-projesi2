...
window.duelloAt = function(hedefIsim) { if(hedefIsim === aktifOgrenci) return alert("Kendine meydan okuyamazsın!"); if(confirm(`${hedefIsim} isimli arkadaşına 50 XP'sine meydan okumak istiyor musun?`)) { socket.emit('duello_teklif_et', { gonderen: aktifOgrenci, hedef: hedefIsim, miktar: 50, kocKodu: kocKodu }); alert("Meydan okuma gönderildi! Bekleniyor..."); document.getElementById('duelloModal').style.display = 'none'; } };
socket.on('duello_istegi_geldi', (veri) => { if(veri.hedef === aktifOgrenci) { bekleyenDuelloRakip = veri.gonderen; document.getElementById('duelloIstekMetni').innerText = `${veri.gonderen} sana 50 XP'sine meydan okuyor!`; document.getElementById('duelloIstekModal').style.display = 'flex'; sesCal(); } });
window.duelloKabulEt = function() { socket.emit('duello_kabul_edildi', { gonderen: bekleyenDuelloRakip, hedef: aktifOgrenci, miktar: 50, kocKodu: kocKodu }); document.getElementById('duelloIstekModal').style.display = 'none'; };
socket.on('duello_basladi', (veri) => { if(veri.o1 === aktifOgrenci || veri.o2 === aktifOgrenci) { alert("⚔️ DÜELLO BAŞLADI! İlk görevi bitiren kazanır!"); sesCal(); } });
socket.on('duello_sonucu', (veri) => { if(veri.kazanan === aktifOgrenci) { alert(`🏆 TEBRİKLER! Düelloyu sen kazandın. +${veri.miktar} XP`); sesCal(); } else if(veri.kaybeden === aktifOgrenci) { alert(`💀 MAALESEF! Düelloyu ${veri.kazanan} kazandı. -${veri.miktar} XP kaybettin.`); } });

let anlikXp = 0; const display = document.getElementById('display'), startBtn = document.getElementById('startBtn'), pauseBtn = document.getElementById('pauseBtn');
let tInterval, running = localStorage.getItem(`${aktifOgrenci}_running`) === 'true', mode = localStorage.getItem(`${aktifOgrenci}_mode`) || 'normal';
if(document.getElementById('mainTitle')) document.getElementById('mainTitle').innerHTML = `Hoş Geldin, ${aktifOgrenci}! 🎯`;
...
socket.on('kaynaklari_yukle', (liste) => { let kutu = document.getElementById('kaynakListesi'); if(!kutu) return; kutu.innerHTML = ''; if(liste.length === 0) { kutu.innerHTML = '<div style="color: #94a3b8; font-style: italic;">Henüz kaynak yok.</div>'; return; } liste.forEach(k => { kutu.innerHTML += `<div class="kaynak-item">${k.baslik} <a href="${k.url}" target="_blank">🔗 Kaynağa Git</a></div>`; }); });
socket.on('yeni_kaynak_eklendi', (kaynak) => { sesCal(); socket.emit('join_room', kocKodu); });

window.avatarSec = function(ikon) { document.getElementById('aktifAvatar').innerHTML = ikon; document.getElementById('avatarModal').style.display = 'none'; socket.emit('avatar_guncelle', { ogrenciAd: aktifOgrenci, avatar: ikon, kocKodu: kocKodu }); };
window.netKaydet = function() { let tur = document.getElementById('sinavTuru').value; let net = document.getElementById('netSkoru').value; if(!net) return; socket.emit('net_ekle', { ogrenciAd: aktifOgrenci, sinavTuru: tur, netSkoru: Number(net), kocKodu: kocKodu }); document.getElementById('netSkoru').value = ''; document.getElementById('netModal').style.display = 'none'; alert("Netiniz iletildi! 🚀"); };
window.odulAl = function(odulIsmi, bedel) { if(anlikXp < bedel) return alert("Yetersiz XP!"); if(confirm(`${odulIsmi} almak istiyor musun?`)) { socket.emit('odul_satin_al', { ogrenciAd: aktifOgrenci, odul: odulIsmi, bedel: bedel, kocKodu: kocKodu }); document.getElementById('marketModal').style.display = 'none'; } };

function gostergeyiGuncelle(sureMs) { if(mode === 'normal') { let hours = Math.floor((sureMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)), minutes = Math.floor((sureMs % (1000 * 60 * 60)) / (1000 * 60)), seconds = Math.floor((sureMs % (1000 * 60)) / 1000); display.innerHTML = (hours < 10 ? "0"+hours : hours) + ":" + (minutes < 10 ? "0"+minutes : minutes) + ":" + (seconds < 10 ? "0"+seconds : seconds); return false; } else { let val = parseInt(document.getElementById('pomoInput').value) || 25, kalan = (val * 60 * 1000) - sureMs; if(kalan <= 0) { display.innerHTML = "00:00"; return true; } let minutes = Math.floor((kalan % (1000 * 60 * 60)) / (1000 * 60)), seconds = Math.floor((kalan % (1000 * 60)) / 1000); display.innerHTML = (minutes < 10 ? "0"+minutes : minutes) + ":" + (seconds < 10 ? "0"+seconds : seconds); return false; } }

window.setMode = function(m) { if(running) return; mode = m; localStorage.setItem(`${aktifOgrenci}_mode`, m); document.getElementById('modeNormal').className = m === 'normal' ? 'mode-btn mode-active' : 'mode-btn mode-passive'; document.getElementById('modePomo').className = m === 'pomodoro' ? 'mode-btn mode-active' : 'mode-btn mode-passive'; if(m === 'pomodoro') { document.getElementById('pomoInput').style.display = 'block'; } else { document.getElementById('pomoInput').style.display = 'none'; } };

function startTimer() { if (!running) { localStorage.setItem(`${aktifOgrenci}_startTime`, new Date().getTime()); localStorage.setItem(`${aktifOgrenci}_running`, 'true'); tInterval = setInterval(() => { let diff = new Date().getTime() - parseInt(localStorage.getItem(`${aktifOgrenci}_startTime`)) + (parseInt(localStorage.getItem(`${aktifOgrenci}_${aktifDersString}_savedTime`)) || 0); if (gostergeyiGuncelle(diff) && mode === 'pomodoro') { sesCal(); pauseTimer(true); } socket.emit('sure_guncelle', { ogrenciAd: aktifOgrenci, sure: display.innerHTML, kocKodu: kocKodu }); }, 1000); running = true; startBtn.style.display = 'none'; pauseBtn.style.display = 'block'; document.getElementById('statusText').innerHTML = "🟢 Odak modu aktif!"; document.getElementById('statusText').style.backgroundColor = "#d1fae5"; document.getElementById('statusText').style.color = "#059669"; document.getElementById('pomoInput').disabled = true; socket.emit('ogrenci_derse_basladi', { ogrenciAd: aktifOgrenci, ders: aktifDersString, mesaj: mode === 'pomodoro' ? 'Pomodoro Başlattı!' : 'Kronometre Başlattı!', kocKodu: kocKodu }); } }
...
window.resimGonder = function(input) { if(input.files && input.files[0]) { const file = input.files[0]; if(file.size > 2.5 * 1024 * 1024) return alert("Resim çok büyük!"); const reader = new FileReader(); reader.onload = function(e) { socket.emit('chat_mesaji_gonder', { gonderen: aktifOgrenci, mesaj: e.target.result, rol: 'ogrenci', tip: 'resim', kocKodu: kocKodu }); input.value = ''; }; reader.readAsDataURL(file); } };
function chatEkranaBas(veri) { const body = document.getElementById('chatBody'); let benMiyim = (veri.gonderen === aktifOgrenci), renk = benMiyim ? '#10b981' : '#e2e8f0', yaziRengi = benMiyim ? 'white' : '#1e293b', hizalama = benMiyim ? 'flex-end' : 'flex-start', ikon = veri.rol === 'koc' ? '👨‍🏫' : '💬'; let icerik = veri.tip === 'resim' ? `<img src="${veri.mesaj}" style="max-width: 100%; border-radius: 8px; margin-top: 5px; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" onclick="window.open('${veri.mesaj}', '_blank')">` : veri.mesaj; body.innerHTML += `<div style="align-self: ${hizalama}; background: ${renk}; color: ${yaziRengi}; padding: 10px 14px; border-radius: 14px; font-size: 13px; max-width: 80%; line-height: 1.4; margin-bottom: 10px; font-weight: 700;"><div style="font-size: 10px; font-weight: 900; margin-bottom: 4px; opacity: 0.8;">${ikon} ${veri.gonderen} • ${veri.saat}</div>${icerik}</div>`; body.scrollTop = body.scrollHeight; }
socket.on('eski_chat_yukle', (gecmis) => { document.getElementById('chatBody').innerHTML = ''; gecmis.forEach(msg => chatEkranaBas(msg)); });
socket.on('yeni_chat_mesaji', (msg) => { chatEkranaBas(msg); const body = document.getElementById('chatBody'); if(msg.gonderen !== aktifOgrenci) { sesCal(); if(body.style.display !== 'flex' && document.getElementById('chatUyari')) document.getElementById('chatUyari').style.display = 'flex'; } });
...
window.botMesajGonder = function() { const input = document.getElementById('botInput'); let mesajMetni = input.value.trim(); if(mesajMetni !== '') { const body = document.getElementById('botBody'); body.innerHTML += `<div style="align-self: flex-end; background: #8b5cf6; color: white; padding: 10px 14px; border-radius: 14px; font-size: 13px; max-width: 80%; line-height: 1.4; margin-bottom: 10px; font-weight: 700;">${mesajMetni}</div>`; body.scrollTop = body.scrollHeight; socket.emit('ogrenci_chatbot_mesaji', { ogrenciAd: aktifOgrenci, mesaj: mesajMetni, ders: aktifDersString, kocKodu: kocKodu }); input.value = ''; } };
socket.on('chatbot_cevabi', (cevapMetni) => { sesCal(); const body = document.getElementById('botBody'); body.innerHTML += `<div style="align-self: flex-start; background: #e2e8f0; color: #1e293b; padding: 10px 14px; border-radius: 14px; font-size: 13px; max-width: 80%; line-height: 1.4; margin-bottom: 10px; font-weight: 700;"><div style="font-size: 10px; font-weight: 900; margin-bottom: 4px; opacity: 0.8;">🤖 SincApp AI</div>${cevapMetni}</div>`; body.scrollTop = body.scrollHeight; });